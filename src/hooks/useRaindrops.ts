...
// -2 (Non-lus) est converti ici en search "status:unread" avec collection
// Tous (0) ; -1 (non classés) et -99 (corbeille) passent tels quels ;
// -3 (Favoris) traverse brut en T4 — sa conversion (important + collection 0)
// arrive avec la vue TopBar (Task 6).
const UNREAD = -2;
  ...
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      api.get<Paginated<RaindropItem>>("/api/raindrops", {
        collection_id: q.collectionId === UNREAD ? 0 : q.collectionId,
        search: q.search ?? (q.collectionId === UNREAD ? "status:unread" : undefined),
        sort: q.sort,

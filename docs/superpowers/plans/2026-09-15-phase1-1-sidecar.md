...
- Codes d'erreur uniformes : `MCP_TIMEOUT` → HTTP 504, `MCP_CRASHED` → 503, `RATE_LIMITED` → 429, `RAINDROP_API` → 502, `INVALID_INPUT` → 400. **Exception bénie** : l'auth locale émet `INVALID_INPUT` avec les statuts HTTP dédiés **401** (sans/mauvais Bearer) et **403** (Origin non locale) — vocabulaire fermé à 5 codes, le front traite 401/403 par statut.
...
- **Plan 3 (shell Tauri)** : spawn `node dist-sidecar/sidecar/index.js` (build `tsc` — imports `.js` déjà nodenext-compatibles) avec `MCP_RAINDROPIO_TOKEN` (Keychain) et `LOCAL_API_TOKEN` (généré éphémère) ; lit le lockfile `sidecar.json` `{port, token, pid}` — **`port: 0` signifie « binding en cours » : poller jusqu'à `port > 0`** (le claim précoce du pid empêche le double-spawn pendant le bind) ; écran premier lancement avant tout spawn.

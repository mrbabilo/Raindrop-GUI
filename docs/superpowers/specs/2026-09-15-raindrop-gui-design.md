...
> ⚠️ **Restauration — point ouvert (relecture API du 2026-09-16).** L'endpoint `POST /raindrops/unrestore` utilisé par `sidecar/direct/raindropRest.ts` **ne figure pas dans l'API publique** et n'a jamais été appelé pour de vrai (le test monte un `fetch` mocké). Deux issues, à trancher avant les Tasks 8/13/15 du plan 2 : **(a)** vérifier en réel que l'endpoint répond — il est vraisemblablement celui de l'app web, et c'est le seul qui restaure **vers la collection d'origine** ; **(b)** basculer sur la voie documentée `PUT /raindrops/-99 {ids, collection:{$id}}`, qui impose de **choisir une collection cible** — ce n'est alors plus la même fonctionnalité : soit l'interface demande la destination, soit le sidecar mémorise la collection d'avant-corbeille.
...
- Archivage de pages (link rot) : capacité **Pro côté Raindrop**, non exposée par le MCP — mais **bien exposée par l'API REST** (relu le 2026-09-16, correction d'une affirmation antérieure) : `GET /raindrop/{id}/cache` pour la copie permanente, et le champ `cache.status` (`ready`, `retry`, `failed`, `invalid-origin`, `invalid-timeout`, `invalid-size`) parmi les champs d'un raindrop. Rien ne bloque techniquement : ce n'est plus une veille, c'est une décision. ⚠️ **Non tranché** : la doc ne donne aucun exemple de réponse de liste, donc on ignore si `cache` figure dans la réponse de `search_raindrops` (gratuit, le snapshot le porterait déjà) ou seulement à l'unité (une requête par lien mort). À vérifier avant de chiffrer. Alternative locale : un module type **ArchiveBox** piloté en job (piste relevée via gosuki).
